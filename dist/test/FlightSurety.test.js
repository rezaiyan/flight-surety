...
describe('FlightSuretyApp', function () {
    ...
    beforeEach(async function () {
        ...
        await flightSuretyApp.connect(airlines[0].signer).fund({ value: hardhat_1.ethers.parseEther('10') });
    ...
    it('Should return true for operational status initially', async function () {
        (0, chai_1.expect)(await flightSuretyApp.isOperational()).to.equal(true);
    });
    describe('FlightSuretyApp Airline Registration', function () {
        const registerAndFundAirline = async (airlineToRegister, registeredBy) => {
            ...
            await flightSuretyApp.connect(airlineToRegister.signer).fund({ value: hardhat_1.ethers.parseEther('10') });
        ...
        const expectAirlineRegistration = async (airline, expected, message) => {
            ...
            (0, chai_1.expect)(isRegistered).to.equal(expected, message);
        };
        ...
        it('prevents non-airlines from registering an airline', async function () {
            try {
                ...
                chai_1.expect.fail("Should have thrown an error since deployer is not a registered airline.");
            }
            catch (error) {
                (0, chai_1.expect)(error.message).to.include('Only existing airline may register a new airline');
            }
        });
    ...
    describe('FlightSuretyApp Flight and Insurance Management', function () {
        ...
        const buyInsurance = async (amount) => {
            await flightSuretyApp.connect(passengerAccount).buy(flight, { value: hardhat_1.ethers.parseEther(amount) });
        };
        it('allows an airline to register a flight', async function () {
            ...
            (0, chai_1.expect)(exists).to.equal(true);
        });
        it('prevents a passenger from purchasing insurance with more than 1 ETH', async function () {
            await registerFlight();
            try {
                await buyInsurance('2');
                chai_1.expect.fail("Should not allow insurance purchase over 1 ETH");
            }
            catch (error) {
                (0, chai_1.expect)(error.message).to.include('Cannot buy more than 1 ETH of insurance');
            }
        });
        it('credits insurees if flight is late due to airline fault', async function () {
            ...
            (0, chai_1.expect)(credit).to.equal(hardhat_1.ethers.parseEther('1.5'));
        });
        it('allows passengers to withdraw funds as credit after flight delay', async function () {
            ...
            (0, chai_1.expect)(remainingCredit).to.equal(hardhat_1.ethers.parseEther('0'));
        });
    });
    describe("FlightSuretyApp Oracle Logic", function () {
        ...
        it("should allow oracles to register", async function () {
            ...
            (0, chai_1.expect)(indexes.length).to.equal(3);
        });
        it("should emit OracleRequest event on fetchFlightStatus", async function () {
            registerFlight();
            await (0, chai_1.expect)(flightSuretyApp.fetchFlightStatus(airlines[0].address, flight, timestamp))
        ...
        it("oracles can submit responses and process flight status", async function () {
            ...
            (0, chai_1.expect)(flightStatus.toString()).to.equal(STATUS_CODE_LATE_AIRLINE.toString());
        });
    });
});
